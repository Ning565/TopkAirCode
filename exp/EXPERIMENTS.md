...
实验一、实验二默认使用统一物理/隐私 profile：`epsilon=1e10`、`sigma0=0.01`、`Pmax=1e6`、`ADC gamma=1.5`、`element_clip=0.02`、`error_feedback_methods=topk,randk`。该 profile 用于验证压缩率先提升准确率、随后因隐私/信道噪声瓶颈下降的主线，并采用带 error feedback 的稀疏 baseline 以避免人为削弱 Rand-k。
