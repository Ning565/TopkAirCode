...
class Config:
    seed: int = 2026
    device: str = "cuda:3"
    output_dir: str = "logs/experiments/full_system/results"
...
class Experiment:
    ...
    def run_method(self, method: str):
        ...
        base_lr = cfg.lr_femnist if self.dataset_name == "femnist" else cfg.lr_mnist
        local_steps = cfg.local_steps_femnist if self.dataset_name == "femnist" else cfg.local_steps_mnist
        rows = []
        for r in range(cfg.rounds):
            lr = max(base_lr * (cfg.lr_decay ** r), cfg.min_lr)
...
def main():
    ...
    parser.add_argument("--lr-mnist", type=float, default=Config.lr_mnist)
    parser.add_argument("--lr-femnist", type=float, default=Config.lr_femnist)
    ...
    parser.add_argument("--optimizer-momentum", type=float, default=Config.optimizer_momentum)
    parser.add_argument("--optimizer-weight-decay", type=float, default=Config.optimizer_weight_decay)
    ...
    with open(out_dir / "summary.md", "w", encoding="utf-8") as f:
        ...
        f.write(
            f"- optimizer：momentum={cfg.optimizer_momentum}，weight_decay={cfg.optimizer_weight_decay}，"
            f"lr_decay={cfg.lr_decay}，min_lr={cfg.min_lr}\n"
        )
        ratio_lines = []
        for dataset in cfg.datasets:
            for method in cfg.methods:
