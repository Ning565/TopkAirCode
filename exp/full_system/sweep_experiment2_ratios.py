...
`exp/full_system/run.py`, then summarized from the generated CSV files.  This
...
def read_metrics(path: Path, target_accuracy: float) -> Dict:
    rows = []
    ...
    for row in sorted(rows, key=lambda item: item["round_i"]):
...
def collect(jobs: Iterable[Dict], target_accuracy: float) -> List[Dict]:
    result = []
    for job in jobs:
        ...
        if not metrics_path.exists():
            ...
            continue
        row = read_metrics(metrics_path, target_accuracy)
...
def write_markdown(path: Path, rows: List[Dict], args) -> None:
    ...
    with path.open("w", encoding="utf-8") as handle:
        ...
        handle.write("| 方法 | k/d | final | best | b* |\n")
        handle.write("|---|---:|---:|---:|---:|\n")
        for method in METHOD_ORDER:
...
def main() -> None:
    ...
    parser.add_argument("--topk-ratios", default="0.10,0.15,0.20,0.25")
    parser.add_argument("--randk-ratios", default="0.20,0.35,0.50,0.65")
    ...
    parser.add_argument("--output-dir", default="exp/full_system/experiment2_ratio_sweep")
    ...
    rows = collect(jobs, args.target_accuracy)
