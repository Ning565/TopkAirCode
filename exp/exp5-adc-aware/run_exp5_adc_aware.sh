...
RUN_TAG="${RUN_TAG:-adcaware_seed${SEED}_r${ROUNDS}_$(date +%Y%m%d_%H%M%S)}"
OUT_DIR="${OUT_DIR:-logs/experiments/exp5-adc-aware/${RUN_TAG}}"
LOG_DIR="${LOG_DIR:-logs/experiments/exp5-adc-aware}"
...

{
  ...
  printf "cd %q\n" "$ROOT_DIR"
  printf "%q " "$PYTHON_BIN" exp/exp5-adc-aware/sweep_adc_robustness.py \
    --execute \
    --python "$PYTHON_BIN" \
    --device "$DEVICE" \
    --seed "$SEED" \
...
echo "Started experiment-5 ADC-aware ablation"
echo "  pid: $PID"
echo "  log: $LOG_FILE"
echo "  out: $OUT_DIR"
echo "  cmd: $CMD_FILE"
