...
To facilitate the convergence analysis, we first formalize the algorithm flow, the fixed-participation OFDM-AirComp aggregation and receiver dynamic-range model, and the assumptions used in the proof. The aggregation is protected by a per-round hybrid mechanism: channel noise supplies part of the required differential-privacy randomness, and each client additionally injects the minimum artificial noise calibrated in closed form from public quantities whenever channel noise alone is insufficient. Throughout this appendix, the global objective is
\begin{equation}
...
\begin{enumerate}
    ...
    \begin{equation}
        s_{A,i}^t(k)=\mathrm{clip}\big(\tilde v_{A,i}^t(k),c_{\mathrm{tx}}\big),
    \end{equation}
\end{enumerate}
...
\begin{equation}
    \|s_{A,i}^t(k)\|_0\le k,
    \qquad
    \|s_{A,i}^t(k)\|_{\infty}\le c_{\mathrm{tx}}.
\end{equation}
...
\begin{equation}
    ...
    \qquad i=1,\ldots,N,
\end{equation}
where $g_i^t=\sqrt{\beta_i}h_i^t$ is the public frequency-flat channel coefficient in round $t$. In addition to the update, each client draws an artificial-noise vector $a_i^t\sim\mathcal N(\mathbf 0,\sigma_{a,t}^2\mathbf I_d)$, independent across clients and rounds, real-valued, and mapped to the same $d$ grid coordinates as the model update; let $A_{i,\ell}^t[m]$ denote its grid image. Conditioned on successful alignment, the frequency-domain observation is
\begin{equation}
    Y_{A,\ell}^t[m;k,b_t]
    =b_t\sum_{i=1}^{N}\big(S_{A,i,\ell}^t[m;k]+A_{i,\ell}^t[m]\big)+N_\ell^t[m],
\end{equation}
with $N_\ell^t[m]\sim\mathcal{CN}(0,\sigma_{\mathrm{sc}}^2)$. The Gaussian randomness used for the per-round client-level privacy guarantee is the superposition of the real part of the thermal noise and the $N$ aggregated artificial-noise components. Define the round-wise effective noise level
\begin{equation}
...
\end{equation}
so that no additional signaling beyond the existing broadcast of $b_t$ is required. When channel noise alone already meets the privacy requirement the injection vanishes and privacy is purely intrinsic; artificial noise is added only to top up the deficit.

Let $c_{\mathrm{tx}}>0$ be a declared public coordinate threshold that is fixed independently of the optimizer learning-rate schedule. The complete-burst average transmit power of client $i$ is
\begin{equation}
    P_{A,i}^{t}
    =\frac{1}{SM}\sum_{\ell=1}^{S}\sum_{m=0}^{M-1}|X_{A,i,\ell}^t[m]|^2
    =\frac{b_t^2}{SM|g_i^t|^2}\big\|s_{A,i}^t(k)+a_i^t\big\|_2^2,
\end{equation}
and its expectation equals $\frac{b_t^2}{SM|g_i^t|^2}\big(\|s_{A,i}^t(k)\|_2^2+d\sigma_{a,t}^2\big)$.
...
\begin{equation}
    ...
    {c_{\mathrm{tx}}\sqrt{k}}.
\end{equation}
Substituting the calibration~\eqref{eqn:artificial_noise_calibration} at equality and $\Delta^2(k)=4c_{\mathrm{tx}}^2k$ gives $\|s_{A,i}^t(k)\|_2^2+d\sigma_{a,t}^2\le kc_{\mathrm{tx}}^2F$ with the $k$-independent noise power tax
\begin{equation}
    \label{eqn:noise_power_tax}
    F\triangleq 1+\frac{2d}{N m^2(\epsilon)} .
\end{equation}
\begin{remark}[\bf Expected versus Realized Noise Power]
    \label{rem:chi_square}
    The constraint is stated in expectation. Conditional on $s_{A,i}^t(k)$, $\|s_{A,i}^t(k)+a_i^t\|_2^2/\sigma_{a,t}^2$ is noncentral chi-square with $d$ degrees of freedom and noncentrality $\|s_{A,i}^t(k)\|_2^2/\sigma_{a,t}^2$. A specified high-probability per-burst guarantee follows from the Laurent--Massart upper-tail bound: with $\bar\lambda\triangleq kc_{\mathrm{tx}}^2/\sigma_{a,t}^2$ and $t\triangleq\ln(1/(1-\eta))$ for a declared confidence $\eta\in(0,1)$, the relative margin
    \begin{equation}
        M\triangleq\frac{\big(2\sqrt{(d+2\bar\lambda)\,t}+2t\big)\sigma_{a,t}^2}{kc_{\mathrm{tx}}^2+d\sigma_{a,t}^2}
    \end{equation}
    satisfies $\Pr\big[\|s_{A,i}^t(k)+a_i^t\|_2^2\le\mathbb E\|s_{A,i}^t(k)+a_i^t\|_2^2(1+M)\big]\ge\eta$ per burst, uniformly over all realizations with $\|s_{A,i}^t(k)\|_2^2\le kc_{\mathrm{tx}}^2$; discounting the power budget by $(1+M)^{-1}$ upgrades the expected constraint accordingly. In the intrinsic regime $\sigma_{a,t}=0$ the transmit energy is deterministic given the update and no margin is needed.
\end{remark}
...
\end{equation}
Define the intrinsic per-round privacy ceiling
\begin{equation}
...
\end{equation}
which reduces to $B_{\epsilon,\mathrm{ex}}(k)$ at $\sigma_a=0$, is increasing in $\sigma_a$, and equals $+\infty$ once $\sigma_a^2\ge\Delta^2(k)/(2Nm^2(\epsilon))$: with sufficient injected noise the privacy requirement no longer restricts the scaling. By the calibration~\eqref{eqn:artificial_noise_calibration}, $\sigma_{a,t}=0$ if and only if $b_t\le B_{\epsilon,\mathrm{ex}}(k)$, recovering the intrinsic-noise regime of prior power-control designs. Provisioning the power budget for the maximal injection level $\bar\sigma_a\triangleq\Delta(k)/(\sqrt{2N}\,m(\epsilon))$ defines the noise-taxed power ceiling
\begin{equation}
...
\end{equation}
with $\sigma_{a,t}$ given by~\eqref{eqn:artificial_noise_calibration} evaluated at $b_t=B_{P,\mathrm{art}}^t(k)$. Thus $b_t$ may vary across rounds through the current public channel state, but it does not depend on current private update norms and requires no cross-round privacy-budget allocation.
\begin{proposition}[\bf Closed-Form Evaluation of the Min Rule]
    \label{prop:min_closed_form}
    In every round, $b_t^\star(k)=B_{P,\mathrm{art}}^t(k)=B_P^t(k)/\sqrt F$. In the top-up regime the calibration~\eqref{eqn:artificial_noise_calibration} holds with equality, hence $B_{\epsilon,\mathrm{art}}(k;\sigma_{a,t})=B_{P,\mathrm{art}}^t(k)$ exactly: the injected noise raises the privacy ceiling precisely to the power ceiling and both branches of the min coincide. In the intrinsic regime $\sigma_{a,t}=0$ and $B_{\epsilon,\mathrm{art}}=B_{\epsilon,\mathrm{ex}}(k)\ge B_{P,\mathrm{art}}^t(k)$, so the noise-taxed power branch is active. This provisioning is conservative relative to the classical intrinsic-only rule because it reserves the maximal artificial-noise budget even when the realized injection vanishes.
\end{proposition}
\begin{proof}
    The calibration holds with equality if and only if $b_t^2\Delta^2(k)=m^2(\epsilon)\big(\sigma_{\mathrm{sc}}^2+2Nb_t^2\sigma_{a,t}^2\big)$, which rearranges to $b_t=B_{\epsilon,\mathrm{art}}(k;\sigma_{a,t})$ by~\eqref{eqn:privacy_ceiling_art}; evaluating the calibration at $b_t=B_{P,\mathrm{art}}^t(k)$ then gives both branches of~\eqref{eqn:per_round_scaling} equal to $B_{P,\mathrm{art}}^t(k)$. If the maximum in~\eqref{eqn:artificial_noise_calibration} is inactive, then $\sigma_{a,t}=0$ and $B_{P,\mathrm{art}}^t(k)\le B_{\epsilon,\mathrm{ex}}(k)$, so the power branch is selected.
\end{proof}
...
\end{equation}
After the unitary FFT, extraction of the original bins, and inverse oversampling normalization, let $\mathbf R_{\mathrm{clip},A,\mathcal D}^t$ be the concatenated frequency-domain residual. The equivalent receiver dynamic-range aggregation error and effective-noise error are
\begin{equation}
    e_{\mathrm{dr},A}^t
...
\end{equation}
by the independent Gaussian sum and~\eqref{eqn:effective_noise}.
The physical update is therefore
\begin{equation}
    \label{eqn:physical_update_dr}
    \theta^{t+1}
    =\theta^t+\frac1N\sum_{i=1}^{N}s_{A,i}^t(k)
    +e_{\mathrm{noise}}^t+e_{\mathrm{dr},A}^t.
\end{equation}

Let $\mathcal F_t$ denote the filtration generated by the physical iterate, all error memories, and all randomness before round $t$. The current public channel state and scaling are measurable before transmission. Channel noise is zero mean conditional on this enlarged public history, and the artificial noise is zero mean and independent of all signals conditional on $\mathcal F_t$ and the public $b_t$, so $e_{\mathrm{noise}}^t$ is conditionally zero mean; the radial-limiting residual is not assumed to be zero mean or independent.
...
\begin{assumption}[\bf Receiver Dynamic-Range Residual]
    ...
    \end{equation}
    The aggregation error $e_{\mathrm{dr},A}^t$ is not assumed to be zero mean and is not assumed to be independent of channel noise or local updates. The radial limiting acts on the received waveform including the artificial noise; the pathwise residual energy~\eqref{eqn:eclip_def} is unaffected by this modeling choice.
\end{assumption}

\begin{assumption}[\bf Successful Full-Client Alignment and Public Scaling]
    \label{assum:public_scaling}
    Every counted logical round satisfies $|h_i^t|\ge h_{\mathrm{cut}}$ for all clients and is completed by all $N$ clients under exact pre-equalized alignment. The public scaling is given by~\eqref{eqn:per_round_scaling} together with the public artificial-noise calibration~\eqref{eqn:artificial_noise_calibration}, is independent of the realized current private updates, and satisfies
    \begin{equation}
...
\begin{theorem}[\bf Per-Round Client-Level Privacy]
    ...
    Consider replace-one client adjacency. Conditional on the same preceding public transcript, current public channel state, public $b_t$, and the data-dependent signals of unchanged clients, with the thermal noise and all $N$ artificial-noise vectors acting as mechanism randomness, the noisy AirComp observation in round $t$ satisfies client-level $(\epsilon,\delta)$-DP under the protocol scaling~\eqref{eqn:per_round_scaling} and calibration~\eqref{eqn:artificial_noise_calibration}, in every round and in both regimes.
\end{theorem}
\begin{proof}
    The pre-noise query has sensitivity at most $b_t\Delta(k)$. The real-part Gaussian randomness of the observation has variance $\sigma_{\mathrm{sc}}^2/2+Nb_t^2\sigma_{a,t}^2=\sigma_{\mathrm{eff},t}^2/2$: the artificial noise of every client is data independent, identically distributed under adjacent executions, and superposed with the thermal noise at the receiver, so it is valid mechanism randomness. The order-$\alpha$ Gaussian RDP cost is therefore at most $\alpha b_t^2\Delta^2(k)/\sigma_{\mathrm{eff},t}^2$. The calibration~\eqref{eqn:artificial_noise_calibration} enforces $b_t^2\Delta^2(k)\le m^2(\epsilon)\,\sigma_{\mathrm{eff},t}^2$; applying the standard RDP-to-DP conversion and optimizing over $\alpha>1$ yields $(\epsilon,\delta)$-DP. AGC, radial limiting, inverse-gain restoration, FFT recovery, division by $N$, the public server-side magnitude truncation of Remark~\ref{rem:denoise}, and model updating are post-processing of the noisy observation.
\end{proof}
...
\end{lemma}
\begin{proof}
    Since $|S_{A,i,\ell}^t[m;k]|\le c_{\mathrm{tx}}$,
    \begin{equation}
        |G_{A,\ell}^t[m;k]|\le c_{\mathrm{tx}}\,U_{A,\ell,m}^t(k).
    \end{equation}
    Hence
    \begin{equation}
...
\begin{corollary}[\bf High-Probability Selection-to-Clipping Linkage]
    ...
    \begin{equation}
        ...
        \sum_{\ell=1}^{S}M
        \left(
        \sqrt{\frac{Q}{M}}b_tc_{\mathrm{tx}}\sqrt{\Omega_{A,\ell}^{t}(k)}+
        \nu_\delta-A_{\max}^t
        \right)_+^2.
    \end{equation}
\end{corollary}
...
\begin{lemma}[\bf Effective-Noise Aggregation Error]
    ...
    Under Assumption~\ref{assum:public_scaling} and the calibration~\eqref{eqn:artificial_noise_calibration}, the aggregation noise error $e_{\mathrm{noise}}^t$ is conditionally zero mean given $\mathcal F_t$ and satisfies
    \begin{equation}
...
\end{lemma}
\begin{proof}
    Zero mean and the variance follow from the independent Gaussian sum $\frac1N\sum_{i=1}^Na_i^t+\mathrm{Re}\{\mathbf N_{\mathcal D}^t\}/(b_tN)$ with per-coordinate variance $\sigma_{a,t}^2/N+\sigma_{\mathrm{sc}}^2/(2b_t^2N^2)=\sigma_{\mathrm{eff},t}^2/(2b_t^2N^2)$ by~\eqref{eqn:effective_noise}. The second identity is~\eqref{eqn:artificial_noise_calibration} evaluated in the two regimes.
\end{proof}
...
\begin{theorem}[\bf Dynamic-Range-Aware Convergence Bound]
    ...
    \begin{align}
        ...
        \Phi_A\big(k;\bar{\omega}_A,\beta_A\big)
        +
        \frac{8\Gamma_A(k)Ld}{T\eta\tau N^2}
        \sum_{t=0}^{T-1}\frac{\sigma_{\mathrm{eff},t}^2}{b_t^2}
        \notag\\
        &\quad+
...
\begin{equation}
    \tilde\theta^{t+1}
    =\tilde\theta^t+a_A^t+e_{\mathrm{noise}}^t+e_{\mathrm{dr},A}^t,
\end{equation}
where
\begin{equation}
    a_A^t
...
\begin{align}
    ...
    a_A^t+e_{\mathrm{noise}}^t+e_{\mathrm{dr},A}^t\right\rangle
    \notag\\
    &\quad+
    \frac{L}{2}\mathbb{E}_t\|a_A^t+e_{\mathrm{noise}}^t+e_{\mathrm{dr},A}^t\|_2^2.
\end{align}
Since $e_{\mathrm{noise}}^t$ is conditionally zero-mean, $\mathbb{E}_t\langle\nabla f(\tilde\theta^t),e_{\mathrm{noise}}^t\rangle=0$. No zero-mean or independence property is assumed for $e_{\mathrm{dr},A}^t$. We decompose
\begin{equation}
    \mathbb{E}_t[f(\tilde\theta^{t+1})-f(\tilde\theta^t)]
...
\begin{align}
    ...
    T_{2,A}&\triangleq\frac{L}{2}\mathbb{E}_t\|a_A^t+e_{\mathrm{noise}}^t+e_{\mathrm{dr},A}^t\|_2^2.
\end{align}
...
Because $e_{\mathrm{dr},A}^t$ may be correlated with both the update and channel noise, we separate it using Young's inequality. With a symmetric choice of the Young parameter,
\begin{align}
    \mathbb{E}_t\|a_A^t+e_{\mathrm{noise}}^t+e_{\mathrm{dr},A}^t\|_2^2
    &\le
    2\mathbb{E}_t\|a_A^t+e_{\mathrm{noise}}^t\|_2^2
    +2\mathbb{E}_t\|e_{\mathrm{dr},A}^t\|_2^2
    \notag\\
    &=2\mathbb{E}_t\|a_A^t\|_2^2
    +2\mathbb{E}_t\|e_{\mathrm{noise}}^t\|_2^2
    +2\mathbb{E}_t\|e_{\mathrm{dr},A}^t\|_2^2,
\end{align}
where the cross term between $a_A^t$ and $e_{\mathrm{noise}}^t$ vanishes by the conditional zero mean of the effective noise. Thus
\begin{equation}
    T_{2,A}
    \le
    L\mathbb{E}_t\|a_A^t\|_2^2
    +L\mathbb{E}_t\|e_{\mathrm{noise}}^t\|_2^2
...
\begin{align}
    ...
    \right).
\end{align}
Substituting this estimate, Lemma~\ref{lemma:noise_error}, and Lemma~\ref{lemma:dr_error}, we obtain
\begin{align}
    ...
    +\frac{Ld\,\sigma_{\mathrm{eff},t}^2}{2b_t^2N^2}
...
\begin{align}
    ...
    \notag\\
    &\quad+
    \frac{4Ld}{T\eta\tau N^2}\sum_{t=0}^{T-1}\frac{\sigma_{\mathrm{eff},t}^2}{b_t^2}
    +
...
\begin{corollary}[\bf Joint Utility and Per-Round Privacy]
    ...
    Under Theorem~\ref{thm:main_convergence}, use the scaling and calibration~\eqref{eqn:per_round_scaling},~\eqref{eqn:artificial_noise_calibration}. Then the physical iterates satisfy the convergence bound, and every round individually satisfies replace-one client-level $(\epsilon,\delta)$-DP. No cross-round privacy-budget allocation is required.
\end{corollary}
\begin{proof}
...
The final convergence bound has three groups. The learning-side term collects local drift, stochastic-gradient variance, heterogeneity, error-feedback memory, and pre-transmission clipping bias. The recovered effective-noise term is proportional to
\begin{equation}
    \frac1T\sum_{t=0}^{T-1}\frac{\sigma_{\mathrm{eff},t}^2}{b_t^2N^2},
\end{equation}
...
\end{equation}
There is no client-participation sampling term because every logical round aggregates all $N$ clients. In the active top-up regime, path loss does not enter the noise term: it affects convergence only through the current-round power ceiling, the resulting $b_t^\star(k)$, and the realized waveform.

\begin{remark}[\bf Channel-Invariant Privacy Noise]
    \label{rem:channel_invariant}
    Whenever the top-up is active, Lemma~\ref{lemma:noise_error} gives $\sigma_{\mathrm{eff},t}^2/b_t^2=\Delta^2(k)/m^2(\epsilon)$, so the recovered-noise term equals the deterministic quantity $8\Gamma_A(k)Ld\,\Delta^2(k)/\big(\eta\tau N^2 m^2(\epsilon)\big)$: path loss and fading drop out of the privacy-noise term entirely, and the sparsity--privacy coupling $k/m^2(\epsilon)$ enters the bound explicitly. The per-coordinate recovered noise level is $\sigma_{\mathrm{dp}}=\sqrt2\,c_{\mathrm{tx}}\sqrt k/(N m(\epsilon))$; it falls below the coordinate clipping threshold $c_{\mathrm{tx}}$ exactly when $\epsilon\ge\epsilon_{\mathrm{loose}}(k)=(\sqrt{2k}/N+\sqrt{\ln(1/\delta)})^2-\ln(1/\delta)$. In the intrinsic regime the term reduces to the familiar random quantity $\sigma_{\mathrm{sc}}^2/b_t^2$.
\end{remark}

\begin{remark}[\bf Server-Side Post-Processing Denoising]
    \label{rem:denoise}
    The recovered-noise term charges the effective noise on all $d$ grid coordinates, whereas the true aggregate $\frac1N\sum_i s_{A,i}^t(k)$ is supported on at most $m\triangleq\min\{d,Nk\}$ coordinates: the support union is private, but its cardinality bound is public. The base station may therefore apply the public truncation operator $\mathcal T_m$ that keeps the $m$ largest-magnitude coordinates of the recovered vector and zeroes the rest. Since $\mathcal T_m$ acts on the already-noisy observation and depends only on the public quantities $N$ and $k$, it is post-processing and leaves Theorem~\ref{thm:per_round_privacy} unchanged, with no additional signaling. Its effect on the recovery error decomposes into (i) the noise retained on the kept coordinates, whose expected energy is controlled by the top-$m$ order statistics of $d$ i.i.d.\ Gaussian coordinates and scales as $m\big(1+2\ln(d/m)\big)\,\sigma_{\mathrm{eff},t}^2/(2b_t^2N^2)$ instead of $d\,\sigma_{\mathrm{eff},t}^2/(2b_t^2N^2)$, and (ii) a mis-ranking bias on signal coordinates displaced from the top $m$, which vanishes in the noiseless limit because the aggregate is $m$-sparse by construction. The bias term governs applicability: magnitude ranking is informative only when the per-coordinate recovered noise level $\sigma_{\mathrm{dp}}$ lies below the per-coordinate scale of the aggregate, of order $c_{\mathrm{tx}}/N$; outside this regime the ranking is noise-driven, the truncation suppresses the zero-mean noise energy but also destroys the (much smaller) signal drift, and the untruncated chain---whose noise averages out across rounds in the stochastic recursion---is preferable. The protocol therefore declares $\mathcal T_m$ enabled or disabled from public quantities before training; Theorem~\ref{thm:main_convergence} is stated for the untruncated chain and applies verbatim whenever $\mathcal T_m$ is disabled, while the truncated protocol replaces the noise-dimension factor $d$ by the public quantity $m\big(1+2\ln(d/m)\big)$ at the cost of the audited mis-ranking bias. This receiver-side use of the public sparsity budget follows the sparse-reconstruction reception strategy of over-the-air aggregation designs that recover a sparse aggregate from the noisy observation at the server.
\end{remark}

With the noise-augmented ceilings, the noise-taxed power ceiling still scales as $1/\sqrt{k}$ under the fixed coordinate-clipping rule, while the noise power tax $F$ is $k$-independent. Sparse transmission therefore raises the feasible $b_t^\star(k)$ relative to full transmission and, through $\Delta(k)\propto\sqrt k$, directly reduces the privacy noise level $\sigma_{\mathrm{dp}}=\sqrt2\,c_{\mathrm{tx}}\sqrt k/(Nm(\epsilon))$: the sparsity--privacy coupling enters the recovered-noise term explicitly. At equal $k$, Top-$k$ and Rand-$k$ have the same worst-case replace-one sensitivity ceiling; their distinction is represented by the certified retention coefficient, clipping bias, and support-overlap-dependent waveform residual.
...
\begin{equation}
    b_t^\star(k)=\min\{B_{\epsilon,\mathrm{art}}(k;\sigma_{a,t}),B_{P,\mathrm{art}}^t(k)\},
\end{equation}
which evaluates to $B_P^t(k)/\sqrt F$ by Proposition~\ref{prop:min_closed_form}.
