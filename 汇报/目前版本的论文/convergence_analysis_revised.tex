...
% Appendix-style numbering for equations and figures only
...
\title{Technical Appendix: Proof of Convergence for ADC-Aware Top-$k$ Sparsified AirComp Federated Learning}
\author{Jinning Song and Dong Liu}
\maketitle

\section{Algorithm Flow and Assumptions}

To facilitate the convergence analysis, we first formalize the algorithm flow, the ADC-aware OFDM-AirComp aggregation model, and the assumptions used in the proof. Throughout this appendix, the global objective is
\begin{equation}
...
Each device maintains a local error memory $e_i^t$, initialized as $e_i^{-1}=\mathbf{0}$. The sparsification-then-clipping mechanism proceeds as follows:
\begin{enumerate}
    ...
    \end{equation}
    For $A=\mathrm{Top}\text{-}k$, $\mathcal{S}_A^k$ keeps the $k$ largest-magnitude entries. For $A=\mathrm{Rand}\text{-}k$, it keeps $k$ randomly selected entries. For $A=\mathrm{Full}$, $k=d$ and no sparsification is applied.
...
\subsection{ADC-Aware OFDM-AirComp Aggregation Model}

The sparse vector $s_{A,i}^t(k)$ is mapped onto $S=\lceil d/M\rceil$ OFDM symbols, each containing $M$ data subcarriers. Let $S_{A,i,\ell}^t[m;k]$ denote the entry transmitted by client $i$ on the $m$-th data subcarrier of the $\ell$-th OFDM symbol. If $S_{A,i,\ell}^t[m;k]=0$, client $i$ remains silent on that subcarrier.

Conditioned on successful channel inversion for active transmissions, the frequency-domain AirComp signal is
\begin{equation}
    Y_{A,\ell}^t[m;k,b_t]
    =b_t\sum_{i=1}^{N}S_{A,i,\ell}^t[m;k]+N_\ell^t[m],
\end{equation}
where $b_t>0$ is the AirComp power scaling factor and
\begin{equation}
    N_\ell^t[m]\sim\mathcal{CN}(0,\sigma_0^2)
\end{equation}
is the complex channel noise. The real part of the noise contributes to the recovered real-valued model update. The channel noise is conditionally zero-mean given the round-level filtration defined below.

Let $Q=L_{\mathrm{os}}M$, where $L_{\mathrm{os}}$ is the oversampling factor. Let $\mathbf{Y}_{A,\ell,\mathrm{os}}^t[k,b_t]\in\mathbb{C}^{Q}$ be the zero-padded oversampled frequency-domain vector obtained from the $M$ data subcarriers. With a unitary $Q$-point IFFT, the time-domain ADC input waveform is
\begin{equation}
    y_{A,\ell}^t[n;k,b_t]
    =\mathrm{IFFT}_{Q}^{\mathrm u}\!\left(\mathbf{Y}_{A,\ell,\mathrm{os}}^t[k,b_t]\right)[n],
    \qquad n=0,\ldots,Q-1.
\end{equation}
The ADC applies the radial clipping operator
\begin{equation}
    \mathcal{C}_{\mathrm{adc}}(y)=
    \begin{cases}
        y, & |y|\le A_{\max},\\
        A_{\max}e^{j\angle y}, & |y|>A_{\max}.
    \end{cases}
\end{equation}
The time-domain clipping residual is
\begin{equation}
    r_{\mathrm{clip},A,\ell}^t[n;k,b_t]
    =\mathcal{C}_{\mathrm{adc}}\big(y_{A,\ell}^t[n;k,b_t]\big)-y_{A,\ell}^t[n;k,b_t].
\end{equation}
The exact waveform-level clipping residual energy is
\begin{equation}
    \label{eqn:eclip_def}
    E_{\mathrm{clip},A}^t(k,b_t)
    =\sum_{\ell=1}^{S}\sum_{n=0}^{Q-1}
    \left(|y_{A,\ell}^t[n;k,b_t]|-A_{\max}\right)_+^2.
\end{equation}

After ADC clipping, the receiver applies a unitary FFT. Let $\mathbf{R}_{\mathrm{clip},A,\ell}^t$ denote the frequency-domain transform of the clipping residual on OFDM symbol $\ell$, and let $\mathbf{R}_{\mathrm{clip},A,\mathcal{D}}^t$ denote its projection onto all data subcarriers after discarding padded inactive entries. The equivalent ADC aggregation error is
\begin{equation}
    e_{\mathrm{adc},A}^t
    =\frac{\mathrm{Re}\{\mathbf{R}_{\mathrm{clip},A,\mathcal{D}}^t\}}{b_tN}.
\end{equation}
The channel-noise aggregation error is
\begin{equation}
    e_{\mathrm{ch}}^t=\frac{\mathrm{Re}\{\mathbf{N}_{\mathcal{D}}^t\}}{b_tN},
    \qquad
    \mathbb{E}\|e_{\mathrm{ch}}^t\|_2^2
    =\frac{d\sigma_0^2}{2b_t^2N^2}.
\end{equation}
Therefore, the actual physical global update is
\begin{equation}
    \label{eqn:physical_update_adc}
    \theta^{t+1}
    =\theta^t+\frac1N\sum_{i=1}^{N}s_{A,i}^t(k)
    +e_{\mathrm{ch}}^t+e_{\mathrm{adc},A}^t.
\end{equation}

Let $\mathcal{F}_t$ denote the round-level filtration generated by $\theta^t$, the error memories $\{e_i^{t-1}\}_{i=1}^{N}$, and all randomness before round $t$. It does not include the current-round mini-batch sampling, sparsification randomness, channel noise, or ADC clipping residual. We write
\begin{equation}
    \mathbb{E}_t[\cdot]\triangleq \mathbb{E}[\cdot\mid\mathcal{F}_t].
\end{equation}
Intermediate local-step conditional expectations are handled by the tower property using local histories $\mathcal{H}_i^{t,s-1}$.
...
\begin{assumption}[\bf ADC Residual Measurability and Finite Energy]
    \label{assum:adc_residual}
    The expected ADC clipping residual energy is finite, i.e.,
    \begin{equation}
        \mathbb{E}\left[E_{\mathrm{clip},A}^t(k,b_t)\right]<\infty.
    \end{equation}
    The ADC aggregation error $e_{\mathrm{adc},A}^t$ is not assumed to be zero-mean and is not assumed to be independent of the channel noise or local updates.
\end{assumption}

\begin{assumption}[\bf AirComp Alignment Conditioning]
    \label{assum:no_outage}
    The convergence proof is conditioned on the event that all scheduled active transmissions satisfy the truncated channel inversion criterion and hence the pre-equalized alignment $h_{i,\ell,m}^tw_{i,\ell,m}^t=b_t$ holds.
\end{assumption}

Let $\mathcal{A}_t$ denote the successful alignment event in round $t$ and define
\begin{equation}
    \mathcal{A}_{0:T-1}\triangleq \bigcap_{t=0}^{T-1}\mathcal{A}_t.
\end{equation}
All convergence statements are understood under the probability measure conditioned on $\mathcal{A}_{0:T-1}$. If outage events are retained explicitly, an additional outage aggregation error term should be added to the final bound.
...
\begin{lemma}[\bf ADC Aggregation Error Bound]
    \label{lemma:adc_error}
    Under the unitary FFT/IFFT operations and the ADC clipping residual definition, the equivalent ADC aggregation error satisfies the pathwise inequality
    \begin{equation}
    ...
    Consequently,
    \begin{equation}
        \mathbb{E}_t\|e_{\mathrm{adc},A}^t\|_2^2
        \le
...
\begin{proof}
    ...
    \end{equation}
    Parseval's theorem under unitary FFT/IFFT normalization gives
    \begin{equation}
        \sum_{\ell=1}^{S}\|\mathbf{R}_{\mathrm{clip},A,\ell}^t\|_2^2
        =\sum_{\ell=1}^{S}\|\mathbf{r}_{\mathrm{clip},A,\ell}^t\|_2^2
        =E_{\mathrm{clip},A}^t(k,b_t).
...
\begin{lemma}[\bf Selection-to-Peak Stress Bound]
    ...
    \begin{equation}
        ...
        \le
        b_t^2\eta^2\tau^2C^2\Omega_{A,\ell}^t(k).
    \end{equation}
\end{lemma}
\begin{proof}
    ...
    \begin{equation}
        ...
        =\|\mathbf{G}_{A,\ell}^t(k)\|_2^2
        \le \eta^2\tau^2C^2\Omega_{A,\ell}^t(k).
    \end{equation}
...
\begin{lemma}[\bf Pairwise Support Overlap Identity]
    \label{lemma:overlap}
    Let $\mathcal{K}_{A,i}^t(k)$ be the selected support of device $i$ in round $t$, and let $\mathcal{I}_{A,i}^t(k)\subseteq\mathcal{K}_{A,i}^t(k)$ be the actually active nonzero support after sparsification and pre-transmission clipping. Define
    \begin{equation}
...
\begin{proof}
    ...
    \begin{equation}
        ...
        b_t\eta\tau C\sqrt{\Omega_{A,\ell}^{t}(k)}+\nu_\delta.
    \end{equation}
    Since $x\mapsto(x-A_{\max})_+^2$ is nondecreasing for $x\ge0$, summing the resulting per-sample clipping upper bound over all $\ell,n$ completes the proof.
\end{proof}

\begin{remark}[\bf Role of the Selection-Dependent Clipping Linkage]
    Corollary~\ref{cor:clipping_linkage} is not used to replace the exact ADC residual in the main convergence theorem. The theorem keeps $\mathbb{E}[E_{\mathrm{clip},A}^{t}(k,b_t)]$ explicitly. The corollary explains how the choice of $A$ influences ADC distortion through the support-overlap path
    \begin{equation}
...
\begin{theorem}[\bf ADC-Aware Convergence Bound]
    \label{thm:main_convergence}
    Suppose Assumptions~\ref{assum:smoothness}--\ref{assum:no_outage} hold, $\tau\ge2$, and the learning rate satisfies
    \begin{equation}
    ...
    \begin{equation}
        \Psi_A(k)
        \triangleq
        2\frac{2\kappa^2+\zeta^2}{\kappa^2+\zeta^2}
        +\eta L\left(\frac12+4L\eta\tau\right)
        \frac{32\tau\kappa^2+8\zeta^2}{\kappa^2+\zeta^2},
    \end{equation}
    with the convention that the variance-dependent ratios are omitted when $\kappa^2+\zeta^2=0$. Let $\Delta f\triangleq f(\theta^0)-f^*$. Define the learning-side term
    \begin{align}
        ...
        +16\Gamma_A(k)\eta\tau L\Psi_A(k)(\kappa^2+\zeta^2)
        \notag\\
        &\quad+
        \frac{16\Gamma_A(k)\Lambda_AR_A(k)}{\eta\tau}
        +R_A(k).
    \end{align}
    Then the physical iterates generated by~\eqref{eqn:physical_update_adc} satisfy
    \begin{align}
        \label{eqn:final_adc_convergence_bound_theorem}
...
\begin{equation}
    ...
    \qquad t\ge0,
\end{equation}
with $\bar e^{-1}=\mathbf{0}$. Equivalently, $\tilde\theta^{t+1}=\theta^{t+1}+\bar e^t$. Using~\eqref{eqn:transmit_signal} and the physical update~\eqref{eqn:physical_update_adc}, the error memories cancel out:
\begin{equation}
    \tilde\theta^{t+1}
    =\tilde\theta^t+a_A^t+e_{\mathrm{ch}}^t+e_{\mathrm{adc},A}^t,
\end{equation}
where
\begin{equation}
    a_A^t
...
\begin{align}
    ...
    a_A^t+e_{\mathrm{ch}}^t+e_{\mathrm{adc},A}^t\right\rangle
    \notag\\
    &\quad+
    \frac{L}{2}\mathbb{E}_t\|a_A^t+e_{\mathrm{ch}}^t+e_{\mathrm{adc},A}^t\|_2^2.
\end{align}
Since $e_{\mathrm{ch}}^t$ is conditionally zero-mean, $\mathbb{E}_t\langle\nabla f(\tilde\theta^t),e_{\mathrm{ch}}^t\rangle=0$. No zero-mean or independence property is assumed for $e_{\mathrm{adc},A}^t$. We decompose
\begin{equation}
    \mathbb{E}_t[f(\tilde\theta^{t+1})-f(\tilde\theta^t)]
    \le T_{1,A}+T_{\mathrm{adc,lin},A}+T_{2,A},
\end{equation}
where
\begin{align}
    T_{1,A}&\triangleq\mathbb{E}_t\langle\nabla f(\tilde\theta^t),a_A^t\rangle,\\
    T_{\mathrm{adc,lin},A}&\triangleq\mathbb{E}_t\langle\nabla f(\tilde\theta^t),e_{\mathrm{adc},A}^t\rangle,\\
    T_{2,A}&\triangleq\frac{L}{2}\mathbb{E}_t\|a_A^t+e_{\mathrm{ch}}^t+e_{\mathrm{adc},A}^t\|_2^2.
\end{align}

\subsection{Bounding $T_{1,A}$, $T_{\mathrm{adc,lin},A}$, and $T_{2,A}$}

\subsubsection{Descent Directions $T_{1,A}$ and $T_{\mathrm{adc,lin},A}$}
...
For the linear ADC term, Young's inequality and Lemma~\ref{lemma:adc_error} imply
\begin{align}
    \label{eqn:t_adc_lin_bound}
    T_{\mathrm{adc,lin},A}
    &\le
...
Because $e_{\mathrm{adc},A}^t$ may be correlated with both the update and the channel noise, we separate it using Young's inequality. With a symmetric choice of the Young parameter,
\begin{align}
    \mathbb{E}_t\|a_A^t+e_{\mathrm{ch}}^t+e_{\mathrm{adc},A}^t\|_2^2
    &\le
    2\mathbb{E}_t\|a_A^t+e_{\mathrm{ch}}^t\|_2^2
    +2\mathbb{E}_t\|e_{\mathrm{adc},A}^t\|_2^2
    \notag\\
...
\begin{equation}
    ...
    +L\mathbb{E}_t\|e_{\mathrm{adc},A}^t\|_2^2.
\end{equation}
...
\begin{align}
    ...
    \right).
\end{align}
Substituting this estimate, the channel-noise variance, and Lemma~\ref{lemma:adc_error}, we obtain
\begin{align}
...
\end{equation}
Combining~\eqref{eqn:t1_bound_new},~\eqref{eqn:t_adc_lin_bound}, and~\eqref{eqn:t2_bound_new}, substituting~\eqref{eqn:psi_bound}, and collecting terms gives
\begin{align}
    \mathbb{E}_t[f(\tilde\theta^{t+1})-f(\tilde\theta^t)]
    &\le
    ...
    \notag\\
    &\quad+
    \Lambda_A\mathcal{M}_{A,t-1}
    +\eta^2\tau^2L\Psi_A(k)(\kappa^2+\zeta^2)
    +\mathcal{E}_A(k)
    \notag\\
    &\quad+
    \frac{Ld\sigma_0^2}{2b_t^2N^2}
    +C_{\mathrm{adc}}\frac{\mathbb{E}_t[E_{\mathrm{clip},A}^t(k,b_t)]}{b_t^2N^2},
\end{align}
where
\begin{equation}
    C_{\nabla,A}^{(0)}
...
\begin{equation}
    ...
    (\eta\tau+2L\eta^2\tau^2)\beta_A^2(k),
    \qquad
    C_{\mathrm{adc}}\triangleq \frac{4}{\eta\tau}+L,
\end{equation}
and
\begin{equation}
    \Psi_A(k)
    ...
    \frac{32\tau\kappa^2+8\zeta^2}{\kappa^2+\zeta^2},
\end{equation}
with the convention that variance-dependent ratios are omitted when $\kappa^2+\zeta^2=0$.
...
\begin{equation}
    ...
    \le
    B_A(k,\{b_t\})+
    \frac{8\Lambda_A}{T\eta\tau}\sum_{t=0}^{T-1}\mathbb{E}[\mathcal{M}_{A,t-1}],
\end{equation}
where
\begin{align}
    B_A(k,\{b_t\})
    ...
    +8\eta\tau L\Psi_A(k)(\kappa^2+\zeta^2)
    +\frac{8\mathcal{E}_A(k)}{\eta\tau}
    \notag\\
    &\quad+
    \frac{4L\sigma_0^2d}{T\eta\tau N^2}\sum_{t=0}^{T-1}\frac1{b_t^2}
    +
    \frac{8C_{\mathrm{adc}}}{T\eta\tau N^2}
    \sum_{t=0}^{T-1}
    \frac{\mathbb{E}[E_{\mathrm{clip},A}^t(k,b_t)]}{b_t^2}.
\end{align}
...
\begin{align}
    \label{eqn:final_adc_convergence_bound}
    \frac1T\sum_{t=0}^{T-1}\mathbb{E}\|\nabla f(\theta^t)\|_2^2
    &\le
    \frac{8\Gamma_A(k)L\sigma_0^2d}{T\eta\tau N^2}
    \sum_{t=0}^{T-1}\frac1{b_t^2}
...
\subsection{Interpretation and ADC-Surrogate Interface}

The bound in~\eqref{eqn:final_adc_convergence_bound} has three terms. The first term is the post-recovery AirComp channel-noise error, which decreases as $b_t$ increases. The second term is the exact ADC-induced aggregation error. It is kept as $\mathbb{E}[E_{\mathrm{clip},A}^t(k,b_t)]$ because ADC clipping is a nonlinear waveform-level functional of the OFDM signal. The third term $\Phi_A(k;\bar{\omega}_A,\beta_A)$ collects all learning-side terms, including local drift, stochastic variance, data heterogeneity, error feedback, and pre-transmission clipping bias. The theorem is an exact convergence statement under a fixed effective ADC threshold $A_{\max}$. In a receiver model with AGC/backoff normalization, the same theorem should be read as the fixed-threshold analytical foundation, while the main-text design replaces the waveform-level ADC term by a calibrated normalized-PAPR surrogate for sparsity selection rather than optimizing $b_t$ through the nonlinear ADC functional.

The role of Top-$k$ relative to Rand-$k$ is reflected through two separate mechanism-dependent quantities. First, the certified energy-retention coefficient can satisfy $\bar{\omega}_{\mathrm{Top}}(k)> k/d$ when the update energy is concentrated, while uniform Rand-$k$ has the baseline contraction $\bar{\omega}_{\mathrm{Rand}}(k)=k/d$ under uniform coordinate sampling. A larger certified or calibrated lower-bound coefficient $\bar{\omega}_A(k)$ decreases $\mu_A(k)$ and hence reduces the error-feedback memory contribution in $\Phi_A$. Second, the ADC term is affected by support overlap through Lemma~\ref{lemma:overlap} and Corollary~\ref{cor:clipping_linkage}. Thus, the Top-$k$ advantage over Rand-$k$ should be attributed to importance retention and mechanism-dependent PAPR/ADC behavior, not to a strictly smaller worst-case DP sensitivity under the same $k$ and clipping threshold.

For the main-text optimization, the exact ADC term may be replaced by a calibrated AGC/backoff surrogate
\begin{equation}
...
\end{equation}
where $\bar y_{A,\ell}^t[n;k]$ is the RMS-normalized ADC input waveform and $\gamma$ is the clipping ratio or backoff threshold. This surrogate affects sparsity selection, whereas $b_t$ is primarily determined by the power and privacy constraints.
