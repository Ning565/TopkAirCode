...
\item \emph{Per-Round Intrinsic Client-Level Privacy:} We derive an exact sufficient scaling ceiling under replace-one client adjacency. Every communication round individually satisfies $(\epsilon,\delta)$-DP; no complete-training guarantee with the same parameters is claimed without a separate composition accountant.
...
Let $g_i^t=\sqrt{\beta_i}h_i^t$ be the public client-level channel coefficient in round $t$. A logical learning round is counted only when all $N$ clients satisfy the public success condition $|h_i^t|\ge h_{\mathrm{cut}}$ and complete the scheduled pre-equalized burst. Client $i$ transmits
\begin{equation}
 X_{i,\ell}^t[m]=\frac{b_t}{g_i^t}S_{i,\ell}^t[m],\qquad i=1,\ldots,N.
\end{equation}
The BS observes
\begin{equation}
 Y_\ell^t[m]=b_t\sum_{i=1}^{N}S_{i,\ell}^t[m]+N_\ell^t[m],
 \label{eqn:freq_aircomp}
\end{equation}
where $N_\ell^t[m]\sim\mathcal{CN}(0,\sigma_{\mathrm{sc}}^2)$. The ideal linear recovery is
\begin{equation}
 \hat S_\ell^t[m]=\frac1N\sum_{i=1}^{N}S_{i,\ell}^t[m]
 +\frac{\mathrm{Re}\{N_\ell^t[m]\}}{b_tN},
 \label{eqn:recover}
\end{equation}
and hence
\begin{equation}
 \hat{\mathbf s}_{\mathrm{lin}}^t=\frac1N\sum_{i=1}^{N}\mathbf s_i^t+\mathbf e_{\mathrm{ch}}^t,
 \qquad \mathbf e_{\mathrm{ch}}^t=\frac{\mathrm{Re}\{\mathbf N_{\mathcal D}^t\}}{b_tN},
 \label{eqn:linear_recovery}
\end{equation}
with
\begin{equation}
 \mathbb E\|\mathbf e_{\mathrm{ch}}^t\|_2^2
...
\end{equation}
Using $\|\mathbf s_i^t\|_2\le c_{\mathrm{tx}}\sqrt{k}$, the public ceiling that guarantees $P_i^t\le P_{\mathrm{cap}}$ for every client is
\begin{equation}
 B_P^t(k)=\min_{1\le i\le N}
 \frac{|g_i^t|\sqrt{SM P_{\mathrm{cap}}}}{c_{\mathrm{tx}}\sqrt{k}}.
 \label{eqn:power}
\end{equation}
For replace-one client adjacency, define $\Delta(k)=2c_{\mathrm{tx}}\sqrt{k}$ and the exact sufficient single-round privacy ceiling
\begin{equation}
 B_{\epsilon,\mathrm{ex}}(k)=\frac{\sigma_{\mathrm{sc}}}{\Delta(k)}
 \left(\sqrt{\epsilon+\ln(1/\delta)}-\sqrt{\ln(1/\delta)}\right).
 \label{eqn:B_eps_exact}
\end{equation}
The unique protocol rule used throughout the paper is
\begin{equation}
 b_t=b_t^\star(k)=\min\{B_{\epsilon,\mathrm{ex}}(k),B_P^t(k)\}.
 \label{eqn:bstar_round}
\end{equation}
It depends only on current public CSI and declared system parameters, never on the realized current private update norm. It requires neither future channel knowledge nor a remaining cross-round privacy budget.
...
\begin{equation}
 ...
 +\mathbf e_{\mathrm{ch}}^t+\mathbf e_{\mathrm{dr}}^t.
...
We consider an honest-but-curious BS that follows the declared full-client AirComp protocol but may use each received signal and released global model to infer a client's data. The protected object is one client's complete local dataset, and adjacency is replace-one client adjacency. For the privacy analysis of round $t$, the preceding public transcript, current public CSI, public $b_t$, and data-independent randomness of unchanged clients are conditioned to be identical under adjacent executions. The pre-noise query is
\begin{equation}
...
\STATE Compute $b_t^\star(k)=\min\{B_{\epsilon,\mathrm{ex}}(k),B_P^t(k)\}$ and broadcast $\boldsymbol\theta^t$, $b_t^\star(k)$, and the fixed resource map
...
\STATE Transmit through pre-equalized OFDM-AirComp
\ENDFOR
...
\end{equation}
The BS observes this query plus real Gaussian noise of variance $\sigma_{\mathrm{sc}}^2/2$ per coordinate. Subsequent AGC, radial limiting, inverse-gain restoration, FFT recovery, normalization, and model updating are post-processing.
...
\begin{theorem}[\bf Per-Round Client-Level Intrinsic DP]\label{thm:dp}
Every communication round individually satisfies replace-one client-level $(\epsilon,\delta)$-DP if
\begin{equation}
 0<b_t\le B_{\epsilon,\mathrm{ex}}(k),
 \label{eqn:privacy_exact}
\end{equation}
where $B_{\epsilon,\mathrm{ex}}(k)$ is defined in~\eqref{eqn:B_eps_exact}. Therefore the protocol scaling~\eqref{eqn:bstar_round} is private and power feasible in every round.
\end{theorem}
\begin{proof}
For RDP order $\alpha>1$, the conditional Gaussian mechanism has privacy cost at most $\alpha b_t^2\Delta^2(k)/\sigma_{\mathrm{sc}}^2$. The RDP-to-DP conversion gives $\epsilon(\alpha)=\alpha b_t^2\Delta^2(k)/\sigma_{\mathrm{sc}}^2+\ln(1/\delta)/(\alpha-1)$. Optimizing over $\alpha$ and solving for $b_t$ yields~\eqref{eqn:B_eps_exact}.
\end{proof}

The guarantee is explicitly per communication round. Observing all $T$ rounds generally requires composition and is not claimed to preserve the same $(\epsilon,\delta)$. Since $B_{\epsilon,\mathrm{ex}}(k)\propto1/\sqrt{k}$, sparse upload permits a larger private scaling than full upload. At equal $k$ and clipping threshold, Top-$k$ and Rand-$k$ have the same worst-case sensitivity ceiling.

\begin{remark}[\bf Privacy--Power Coupling]
The current public channel changes $B_P^t(k)$, whereas the declared per-round privacy parameters determine $B_{\epsilon,\mathrm{ex}}(k)$. Their minimum gives a causal channel-adaptive $b_t^\star(k)$ and directly determines recovered noise.
\end{remark}
...
\begin{align}
 ...
 &\quad+\frac{\lambda_{\mathrm{ch}}}{T}\sum_{t=0}^{T-1}\frac1{b_t^2}.
 \label{eqn:calibrated_score}
\end{align}
The normalized learning and dynamic-range statistics are frozen during the scaling search. The single operating-point problem is
\begin{align}
 \min_{k\in\mathcal K,\{b_t>0\}}\quad &\widehat{\mathcal J}_A(k,\{b_t\})\label{eqn:unified_design}\\
 \mathrm{s.t.}\quad &b_t\le B_{\epsilon,\mathrm{ex}}(k),\quad \forall t,\nonumber\\
 &b_t\le B_P^t(k),\quad \forall t.\nonumber
\end{align}
...
\end{proposition}
\begin{proof}
With the calibrated learning and normalized dynamic-range statistics frozen, the only round-$t$ scaling-dependent term is $1/b_t^2$, which is strictly decreasing for $b_t>0$. The optimum is therefore the largest value satisfying the two per-round ceilings.
\end{proof}
...
\begin{align}
 ...
 +\lambda_{\mathrm{clip}}\widehat\beta_A^2(k)
 +\lambda_{\mathrm{dr}}\widehat D_{\mathrm{DR},A}(k;\gamma)\notag\\
 &\quad+\frac{\lambda_{\mathrm{ch}}}{T}\sum_{t=0}^{T-1}\frac1{(b_t^\star(k))^2},
\end{align}
