...
\end{equation}
whose expectation is $\frac{b_t^2}{SM|g_i^t|^2}(\|\mathbf s_i^t\|_2^2+d\sigma_{a,t}^2)$. Conditional on $\mathbf s_i^t$, the realized squared norm is noncentral chi-square after normalization; hence the power ceiling below is an expected-power guarantee. A deterministic or specified high-probability per-burst guarantee requires an explicit noncentral-chi-square concentration margin. Using $\|\mathbf s_i^t\|_2\le c_{\mathrm{tx}}\sqrt{k}$, define
\begin{equation}
