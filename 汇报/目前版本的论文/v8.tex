...
\begin{equation}
	\mathbf{s}_{A,i}^t(k)
	=\operatorname{clip}\big(\tilde{\mathbf{v}}_{A,i}^t(k),c_{\mathrm{tx}}\big),
...
\begin{equation}
	...
	\qquad
	\|\mathbf{s}_{A,i}^t(k)\|_\infty\le c_{\mathrm{tx}}.
\end{equation}
}
...
\end{equation}
Before the model update, the BS may additionally apply a public post-processing truncation: the true aggregate is supported on at most $\min\{d,Nk\}$ coordinates and both $N$ and $k$ are public, so the recovery keeps the $\min\{d,Nk\}$ largest-magnitude coordinates of $\hat{\mathbf s}_{\mathrm{lin}}^t$ and zeroes the rest, following the sparse-reconstruction reception strategy of over-the-air designs that recover a sparse aggregate from the noisy observation at the server~\cite{amiri2020machine,jeon2021compressive}. Acting on the already-noisy observation with public parameters only, the truncation does not affect the privacy guarantee, and it reduces the noise energy entering the model from the $d$-coordinate count in~\eqref{eqn:channel_error_energy} to its top-$\min\{d,Nk\}$ order statistics. Magnitude ranking is informative only when $\sigma_{\mathrm{dp}}$ lies below the per-coordinate scale of the aggregate (of order $c_{\mathrm{tx}}/N$); outside this regime the ranking is noise-driven and the truncation would bias the update, so the protocol declares the truncation enabled or disabled from public quantities before training and the convergence analysis charges the full $d$-dimensional noise term whenever it is disabled.

\subsection{Per-Round Power Constraint and Public Scaling}
Let $c_{\mathrm{tx}}>0$ be a declared public coordinate threshold fixed independently of the optimizer learning-rate schedule. With a unitary IFFT, client $i$ has complete-burst average transmit power
\begin{equation}
 P_i^t=\frac{b_t^2}{SM|g_i^t|^2}\big\|\mathbf s_i^t+\mathbf a_i^t\big\|_2^2,
\end{equation}
whose expectation is $\frac{b_t^2}{SM|g_i^t|^2}(\|\mathbf s_i^t\|_2^2+d\sigma_{a,t}^2)$. Conditional on $\mathbf s_i^t$, the realized squared norm is noncentral chi-square after normalization; the power ceiling below is therefore an expected-power guarantee, and a specified high-probability per-burst guarantee follows by discounting the budget with the explicit Laurent--Massart margin derived in the appendix. Using $\|\mathbf s_i^t\|_2\le c_{\mathrm{tx}}\sqrt{k}$, define
\begin{equation}
...
\end{equation}
Receiver noise randomizes this query before AGC, radial limiting, inverse-gain restoration, FFT recovery, division by $N$, public magnitude truncation, and model release.
...
\small
\begin{algorithmic}[1]
\REQUIRE Sparsity $k$, public coordinate threshold $c_{\mathrm{tx}}$, local steps $\tau$, and step size $\eta$
...
\end{equation}
The BS observes this query plus the real Gaussian randomness formed by the thermal noise of variance $\sigma_{\mathrm{sc}}^2/2$ per coordinate and the $N$ aggregated artificial-noise components, with total per-coordinate variance $\sigma_{\mathrm{eff},t}^2/2$. Subsequent AGC, radial limiting, inverse-gain restoration, FFT recovery, normalization, public magnitude truncation, and model updating are post-processing.

\begin{lemma}[\bf Sparse AirComp Sensitivity]\label{lemma:sen}
For any mechanism satisfying $\|\mathbf s_i^t(k)\|_0\le k$ and $\|\mathbf s_i^t(k)\|_\infty\le c_{\mathrm{tx}}$,
\begin{equation}
 \Delta_2^t(\mathcal Q)\le b_t\Delta(k),\qquad \Delta(k)=2c_{\mathrm{tx}}\sqrt{k}.
 \label{eqn:sensitivity_bound}
\end{equation}
\end{lemma}
\begin{proof}
Under the stated conditioning, adjacent datasets differ in one client $l$, so the query difference is $b_t(\mathbf s_l^t-(\mathbf s_l^t)')$. Both sparse clipped vectors have norm at most $c_{\mathrm{tx}}\sqrt{k}$, and the triangle inequality gives the result even when their selected supports differ.
\end{proof}
...
Since every nonzero entry has magnitude at most $c_{\mathrm{tx}}$,
{
\begin{equation}
	|G_{A,\ell}^t[m;k]|
	\le c_{\mathrm{tx}} U_{A,\ell,m}^t(k).
\end{equation}
}
...
We next develop a convergence bound that explicitly separates the learning-side errors from the physical aggregation errors. Following the decomposition strategy in intrinsic-private sparse AirComp-FL~\cite{zhang2023communication}, the proof first rewrites the update through a virtual sequence that absorbs the local error-feedback memory, and then bounds the per-round descent by the local SGD drift, sparsification residual, pre-transmission clipping bias, effective Gaussian aggregation noise, and radial-clipping distortion. Different from rand-$k$-based PFELS, the proposed analysis keeps the mechanism-dependent retained-energy coefficient of top-$k$ and the waveform-level dynamic-range residual as explicit terms.
...
Let $\mathcal{F}_t$ be the round-level filtration generated by $\boldsymbol{\theta}^t$, the memories $\{\mathbf{e}_i^{t-1}\}_{i=1}^{N}$, and all randomness realized before round $t$. It excludes the current mini-batches, any current sparsification randomness, the current thermal and artificial noises, and the current dynamic-range residual. We write $\mathbb{E}_t[\cdot]=\mathbb{E}[\cdot\mid\mathcal{F}_t]$; conditional expectations inside local SGD are taken with respect to the corresponding local history and then reduced to $\mathbb{E}_t$ by the tower property. This notation is important because the effective Gaussian aggregation noise is conditionally zero-mean, whereas radial-clipping error need not be.
...
\end{align}
with $k_A^\star=\arg\min_{k\in\mathcal K}J_A(k)$. Candidates are additionally screened by the absolute-scale feasibility gate
$\chi_{\mathrm{eff}}(k)=\min\{d,Nk\}\,\sigma_{\mathrm{dp}}^2(k)/\mathbb E\|\bar{\mathbf s}^t(k)\|_2^2\le\chi_{\max}$,
the noise-to-signal energy ratio on the aggregate support union in absolute units, which the per-mechanism normalization of $J_A(k)$ deliberately does not constrain. The server evaluates $B_P^t(k)$ from current public CSI and applies~\eqref{eqn:bstar} causally. No future channel trajectory or cross-round privacy allocation is required.
...
Fig.~\ref{fig:exp2} evaluates test accuracy for privacy budgets from $\epsilon=0.2$ to $3.0$ using the legacy intrinsic-noise implementation. The proposed operating point is higher in these single-run results. \rev{Relative to full-dimensional transmission, element-wise clipping and sparse upload reduce the AirComp sensitivity from the $\mathcal{O}(\sqrt{d})$ scale to the $\mathcal{O}(\sqrt{k})$ scale.} In that legacy mechanism, the per-round exact ceiling $B_{\epsilon,\mathrm{ex}}(k)$ permits a larger $b_t$ for smaller $k$, reducing post-recovery noise. \rev{At matched $k$ and clipping threshold, top-$k$ and common-support rand-$k$ share the same worst-case sparse sensitivity order.} These curves do not validate the new artificial-top-up mechanism or its $\epsilon_{\mathrm{loose}}(k)$ transition; that claim requires the separate OFDM rerun declared above.
