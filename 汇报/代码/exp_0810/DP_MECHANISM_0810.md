...
### 6.1 旧 SNR15 工作点锚定（TODO：反算与之前的量一致）
...
### 6.4 功率约束的高概率口径（影响文档遗留问题）

高斯噪声能量无界，功率陈述为期望口径。实际发送能量是 ‖s_i+a_i‖²；给定
s_i 后，它按 σ_a² 归一化服从带非中心参数 ‖s_i‖²/σ_a² 的非中心卡方分布，
不能只用 ‖a_i‖² 的中心卡方波动代替。若需要指定置信度的逐突发保证，应根据
非中心卡方集中界预留显式功率裕量；当前代码的 `expected_power_utilization` 只作
期望功率诊断，不声称逐噪声实现必然满足上限。
